...
# ========== 新增 CategoryFilterPicker.vue ==========
cat > "$FRONTEND/src/components/CategoryFilterPicker.vue" << 'VUEEOF'
...
# ========== TransactionList.vue:改用 CategoryFilterPicker ==========
python3 << 'PYEOF'
path = "ledger-frontend/src/components/TransactionList.vue"
with open(path) as f:
    content = f.read()

old_import = """import CategoryPicker from './CategoryPicker.vue'
import TagPicker from './TagPicker.vue'"""
new_import = """import CategoryPicker from './CategoryPicker.vue'
...
old_select = """      <select v-model="filterCategoryId" class="filter-input">
        <option value="">所有分類</option>
        <option v-for="c in categories" :key="c.id" :value="c.id">{{ c.name }}</option>
      </select>
      <input
        v-model.number="filterMinAmount\""""
new_select = """      <CategoryFilterPicker v-model="filterCategoryId" :categories="categories" />
      <input
        v-model.number="filterMinAmount\""""
if old_select not in content:
    raise SystemExit("❌ filterCategoryId select 錨點不符")
content = content.replace(old_select, new_select, 1)

with open(path, "w") as f:
    f.write(content)
print("✅ TransactionList.vue 已改用 CategoryFilterPicker(按鍵式搜尋)")
PYEOF

git add -A
git commit -m "feat: 交易篩選列的分類改為按鍵式搜尋(CategoryFilterPicker),取代 80+ 項下拉選單"
